...
final class UserUpdateBatchRequest extends BaseFormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'users.*.id' => 'required|integer|exists:public.users,id',
            'users.*.first_name' => 'sometimes|required|string|max:255',
            'users.*.last_name' => 'sometimes|required|string|max:255',
            'users.*.email' => "sometimes|required|email|max:255|unique:public.users,email",
            'users.*.is_active' => 'boolean',
            'users.*.parent_id' => 'integer|exists:public.users,id',
            'users.*.change_password' => 'nullable|boolean',
            'users.*.password' => 'exclude_unless:change_password,true|required|string|confirmed',
            'users.*.roles.*.id' => 'sometimes|required|integer|min:1|exists:public.roles,id',
            'users.*.affiliate_id' => 'nullable|integer|exists:public.users,id',
